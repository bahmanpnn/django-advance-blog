...
urlpatterns = [
    # registration
    path(
        "registration/", RegistrationAPIView.as_view(), name="registration-api"
    ),
    ...
    # login token
    path(
        "token/login/", MyCustomAuthToken.as_view(), name="custom-token-login"
    ),
    # path('api-token-auth/', views.obtain_auth_token)
    # logout(destroy) token
    path(
        "token/logout/",
        CustomDestroyAuthToken.as_view(),
        name="custom-token-logout",
    ),
    # login jwt
    path(
